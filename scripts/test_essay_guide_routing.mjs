...
const retrieve = (query, sectionBoostId) => {
  ...
  const scored = corpus.chunks.map((chunk) => {
    ...
    if (qYear && (chunk.id?.includes(qYear) || (chunk.keywords || []).includes(qYear))) score += 4;
    if (/\b(trade|openness|exports|imports)\b/.test(qNorm) && chunk.section === "macro") score += 3;
    if (/\b(happened|when|timeline|year|event)\b/.test(qNorm) && chunk.section === "timeline") score += 2;
...
const SHOULD_ANSWER = [
  ...
  "Why did the sovereign rating downgrade in 2017",
  "What happened with COVID unemployment",
  "When did South Africa join the WTO",
